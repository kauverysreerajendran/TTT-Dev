    ...
    for raw_lot_id in lot_ids:
        try:
            ...
            if jig_lot_id:
                ...
                jig_detail = JigDetails.objects.select_related('plating_color').filter(jig_lot_id=jig_lot_id).first()
                if not jig_detail:
                    # Fallback: try with lot_id_quantities
                    jig_detail = JigDetails.objects.select_related('plating_color').filter(
            ...
            # 🔧 CRITICAL FIX: Get plating_color from JigDetails, not TotalStockModel
            if jig_detail and jig_detail.plating_color:
                # JigDetails.plating_color is a string, need to get the Plating_Color object
                try:
                    ...
                    print(f"[SMART POPULATE Zone2] 🎨 Setting plating_color from JigDetails: {plating_color_obj.plating_color} (ID: {plating_color_obj.id})")
                except Plating_Color.DoesNotExist:
                    print(f"[SMART POPULATE Zone2] ❌ Plating_Color '{jig_detail.plating_color}' not found in database")
                    jig_unload_instance.plating_color = total_stock.plating_color
                    print(f"[SMART POPULATE Zone2] 🔄 Fallback to TotalStock plating_color: {total_stock.plating_color}")
            elif total_stock.plating_color:
                jig_unload_instance.plating_color = total_stock.plating_color
                print(f"[SMART POPULATE Zone2] 🎨 Setting plating_color from TotalStock: {total_stock.plating_color} (ID: {total_stock.plating_color.id})")
            else:
                print(f"[SMART POPULATE Zone2] ⚠️ No plating_color found in JigDetails or TotalStock for lot_id: {lot_id}")
...
def JU_Zone_save_jig_unload_tray_ids(request):
    ...
    try:
            ...
            if existing_record:
                ...
                try:
                    ...
                    if not jig_unload_fter_instance.plating_color:
                        print(f"[SMART SAVE] ⚠️ New record {jig_unload_fter_instance.id} has no plating_color, attempting to populate...")
                        # Try to get plating_color from JigDetails associated with any of the lot_ids
                        for lot_id in cleaned_combined_lot_ids:
                            try:
                                jig_detail = JigDetails.objects.filter(
                                ...
                                if jig_detail and jig_detail.plating_color:
                                    ...
                                    print(f"[SMART SAVE] ✅ Fixed plating_color for new record: {plating_color_obj.plating_color}")
                                    break
                            except (Plating_Color.DoesNotExist, Exception) as e:
                                print(f"[SMART SAVE] ⚠️ Error fixing plating_color for lot {lot_id}: {e}")
                                continue
        ...
        return JsonResponse({
            'success': True,
            'message': f'Smart saved {len(saved_trays)} tray records with preserved jig mappings',
...
class JU_Zone_Completedtable(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for idx, unload in enumerate(completed_unloads):
            ...
            # 🔧 ENHANCED: Get jig_qr_id from multiple sources
            ...
            if unload.jig_qr_id:
                ...
                print(f"[DEBUG] Using jig_qr_id from JigUnloadAfterTable: {jig_qr_id}")
            elif first_lot_id:
                ...
                if jig_detail and jig_detail.jig_qr_id:
                    ...
                    print(f"[DEBUG] Found jig_qr_id from JigDetails: {jig_qr_id}")
                else:
...
def JU_Zone_get_model_images(request):
    ...
    try:
        # Fetch polish_stk_no from JigUnloadAfterTable using lot_id
        jig_unload = JigUnloadAfterTable.objects.filter(lot_id=lot_id).first()
        if not jig_unload or not jig_unload.polish_stk_no:
            print("No JigUnloadAfterTable or polish_stk_no found for lot_id:", lot_id)
            return JsonResponse({'success': False, 'image': None})
        polish_stk_no = jig_unload.polish_stk_no
        # Extract model_no before 'X'
        model_no = polish_stk_no.split('X')[0] if 'X' in polish_stk_no else polish_stk_no
        model = ModelMaster.objects.prefetch_related('images').filter(model_no=model_no).first()
        if not model:
            print("No ModelMaster found for model_no:", model_no)
            return JsonResponse({'success': False, 'image': None})
        images = [img.master_image.url for img in model.images.all()]
        print("Fetched images for model_no", model_no, ":", images)
        first_image = images[0] if images else None
        return JsonResponse({'success': True, 'image': first_image})
    except Exception as e:
        print("Error in JU_Zone_get_model_images:", str(e))
        return JsonResponse({'success': False, 'error': str(e), 'image': None})
