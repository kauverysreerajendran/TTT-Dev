...
def populate_jig_unload_fields(jig_unload_instance, lot_ids, jig_lot_id=None):
    ...
    for raw_lot_id in lot_ids:
        try:
            ...
            # Debug logging for plating color
            print(f"[SMART POPULATE] 🎨 Setting plating_color: {total_stock.plating_color} (ID: {total_stock.plating_color.id if total_stock.plating_color else 'None'})")
...
def save_jig_unload_tray_ids(request):
    ...
    try:
        ...
        # DEDUP and existing checks (unchanged)
        ...
        def get_stock_model_data_for_save(lot_id):
            ...
            if 'JLOT-' in lot_id and (':' in lot_id or '-' in lot_id):
                if ':' in lot_id:
                    actual_lot_id = lot_id.split(':')[-1]
                elif '-' in lot_id:
                    parts = lot_id.split('-')
                    actual_lot_id = parts[-1] if len(parts) > 1 else lot_id
                else:
        ...
        # Collect plating/polish/version values (existing logic)
        all_plating_stk_nos = []
        all_polish_stk_nos = []
        all_versions = []
        ...
        # CREATE/UPDATE JigUnloadAfterTable (existing logic)...
        ...
        if final_db_combined_lot_ids and total_case_qty > 0:
            ...
            for record in potential_records:
                if record.combine_lot_ids and any(formatted_id in record.combine_lot_ids for formatted_id in final_db_combined_lot_ids):
            ...
            if existing_record:
                ...
                try:
                    create_data = {
                        'combine_lot_ids': final_db_combined_lot_ids,
        ...
        for i, tray in enumerate(trays):
            ...
            is_top_tray = tray.get('is_top_tray', False)
        ...
        # === NEW: Ensure Jig rows are unlocked (is_loaded -> False) so QR can be reused ===
        try:
            affected_jig_qr_ids = set()

            # 1) From jig_aware_sources (format "JLOT-XXX:LOTID") - DIRECT: jig_lot_id is the jig_qr_id
            if jig_aware_sources:
                for source in jig_aware_sources:
                    if ':' in source:
                        jig_lot_id_part = source.split(':', 1)[0].strip()
                        if jig_lot_id_part:
                            affected_jig_qr_ids.add(jig_lot_id_part)
                            print(f"[SMART SAVE] ✅ Added jig_qr_id from jig_aware_source: {jig_lot_id_part}")

            # 2) From direct jig_lot_id - DIRECT: jig_lot_id is the jig_qr_id
            if jig_lot_id:
                affected_jig_qr_ids.add(jig_lot_id)
                print(f"[SMART SAVE] ✅ Added jig_qr_id from direct jig_lot_id: {jig_lot_id}")

            # 3) From cleaned_combined_lot_ids -> lookup JigDetails that reference these lot ids
            for lot in (cleaned_combined_lot_ids or []):
                jig_details = JigDetails.objects.filter(
                    Q(lot_id=lot) | Q(new_lot_ids__contains=[lot]) | Q(combined_lot_ids__contains=[lot])
                )
                for jig_detail in jig_details:
                    if getattr(jig_detail, 'jig_qr_id', None):
                        affected_jig_qr_ids.add(jig_detail.jig_qr_id)
                        print(f"[SMART SAVE] ✅ Added jig_qr_id from JigDetails for lot {lot}: {jig_detail.jig_qr_id}")

            # 4) If unload record carried a jig_qr_id, include it
            if jig_unload_fter_instance and getattr(jig_unload_fter_instance, 'jig_qr_id', None):
                affected_jig_qr_ids.add(jig_unload_fter_instance.jig_qr_id)
                print(f"[SMART SAVE] ✅ Added jig_qr_id from unload record: {jig_unload_fter_instance.jig_qr_id}")

            # Perform the update: unlock jigs so they can be reused in next cycle
            if affected_jig_qr_ids:
                updated_count = Jig.objects.filter(jig_qr_id__in=list(affected_jig_qr_ids)).update(
                    is_loaded=False,
                    drafted=False,
                    current_user=None,
                    locked_at=None
                )
                print(f"[SMART SAVE] ✅ Unlocked {updated_count} jigs (set is_loaded=False)")
                logger.info(f"Unlocked {updated_count} jigs: {list(affected_jig_qr_ids)}")
            else:
                print("[SMART SAVE] ⚠️ No jig_qr_ids found to unlock")
                logger.warning("No jig_qr_ids found to unlock during unload")

        except Exception as e:
            print(f"[SMART SAVE] Error: failed to clear Jig.is_loaded flags: {e}")
            logger.error(f"Failed to clear Jig.is_loaded flags: {e}", exc_info=True)

        return JsonResponse({
            'success': True,
            'message': f'Unloaded Sucessfully',
...
class JigUnloading_Completedtable(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for idx, unload in enumerate(completed_unloads):
            ...
            # Get first combine_lot_id for jig_qr_id lookup
            first_lot_id = unload.combine_lot_ids[0] if unload.combine_lot_ids else None
            jig_qr_id = None
            if first_lot_id:
                jig_detail = JigDetails.objects.filter(lot_id=first_lot_id).first()
                if jig_detail:
