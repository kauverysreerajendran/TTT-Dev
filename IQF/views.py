...
def _get_all_lot_trays_for_iqf(lot_id):
    """
    Get ALL trays mapped to a lot from ALL relevant sources.
    Returns dict: {tray_id: max_qty_across_sources}
    
    This ensures IQF delink, remaining trays, and rejection allocation logic
    can discover every tray regardless of how the lot arrived at IQF
    (Brass QC rejection, Brass Audit rejection, or direct IQF).
    """
    all_trays = {}
    ...
    def _update(tid, qty):
        if tid:
            try:
                ...
                q = 0
            if q > 0:
                all_trays[tid] = max(all_trays.get(tid, 0), q)

    # Source 1: TrayId master table (most authoritative, excludes delinked trays via lot_id=None)
    for t in TrayId.objects.filter(lot_id=lot_id, delink_tray=False).values('tray_id', 'tray_quantity'):
        _update(t['tray_id'], t['tray_quantity'])

    # Source 2: BrassTrayId (Brass QC stage, exclude delinked)
    for t in BrassTrayId.objects.filter(lot_id=lot_id, delink_tray=False).values('tray_id', 'tray_quantity'):
        _update(t['tray_id'], t['tray_quantity'])

    # Source 3: BrassAuditTrayId (Brass Audit stage, exclude delinked)
    for t in BrassAuditTrayId.objects.filter(lot_id=lot_id, delink_tray=False).values('tray_id', 'tray_quantity'):
        _update(t['tray_id'], t['tray_quantity'])

    # Source 4: IQFTrayId (IQF stage, exclude delinked)
    for t in IQFTrayId.objects.filter(lot_id=lot_id, delink_tray=False).values('tray_id', 'tray_quantity'):
        _update(t['tray_id'], t['tray_quantity'])

    # Source 5: Brass QC rejection scans (may have trays not in above tables)
    for rec in Brass_QC_Rejected_TrayScan.objects.filter(lot_id=lot_id).exclude(
        ...
        _update(rec.rejected_tray_id, rec.rejected_tray_quantity)

    # Source 6: Brass Audit rejection scans
    for rec in Brass_Audit_Rejected_TrayScan.objects.filter(lot_id=lot_id).exclude(
        rejected_tray_id__isnull=True
    ).exclude(rejected_tray_id=''):
        _update(rec.rejected_tray_id, rec.rejected_tray_quantity)

    # Source 7: IQF rejection scans (trays already allocated to IQF rejection)
    for rec in IQF_Rejected_TrayScan.objects.filter(lot_id=lot_id).exclude(
        tray_id__isnull=True
    ).exclude(tray_id=''):
        _update(rec.tray_id, rec.rejected_tray_quantity)
...
class IQFCompleteTableTrayIdListAPIView(APIView):
    def get(self, request):
        ...
        try:
            ...
            all_trays = []
            delinked_trays = []
            # Always define these to avoid NameError in any flow
            scan_qty_map = {}
            scan_top_map = {}
            ...
            # --- PATCH: Strict full accept override ---
            ...
            # --- PATCH: Strict full accept override (IQF Proceed, no rejection) ---
            accepted_store_trays = IQF_Accepted_TrayID_Store.objects.filter(lot_id=lot_id)
            accepted_ids = {t.tray_id for t in accepted_store_trays if t.tray_id}

            # Get lot-level rejection info and lot qty
            ...
            lot_qty = int(total_stock_for_qty.iqf_physical_qty or 0) if total_stock_for_qty else 0

            if lot_total_rejected_qty == 0:
                print("🚨 IQF FULL ACCEPT MODE (NO REJECTION)")
                # Step 1: Get IQF current trays (source of truth)
                iqf_trays = []
                tray_records = IQFTrayId.objects.filter(lot_id=lot_id)
                for tray in tray_records:
                    iqf_trays.append({
                        "tray_id": tray.tray_id,
                        "qty": tray.tray_quantity
                    })
                print(f"📦 IQF INPUT TRAYS: {iqf_trays}")
                # Step 2: Hard reset old data
                IQF_Rejected_TrayScan.objects.filter(lot_id=lot_id).delete()
                IQF_Accepted_TrayID_Store.objects.filter(lot_id=lot_id).delete()
                # Step 3: Save exact same (no change)
                total = 0
                rebuilt_accepted = []
                for t in iqf_trays:
                    ...
                    total += t["qty"]
                print(f"✅ FINAL SAVED (NO CHANGE): total={total}")
                # Step 4: Update lot qty (brass_qc_accepted_qty)
                try:
                    total_stock_obj = TotalStockModel.objects.get(lot_id=lot_id)
                    total_stock_obj.brass_qc_accepted_qty = total
                    total_stock_obj.save(update_fields=["brass_qc_accepted_qty"])
                except Exception as e:
                    print(f"[IQF FULL ACCEPT] Could not update brass_qc_accepted_qty: {e}")
                all_trays.extend(rebuilt_accepted)
                accepted_ids = set([t['tray_id'] for t in iqf_trays])
                # Ensure scan_qty_map and scan_top_map are always defined for later code
                scan_qty_map = {}
                scan_top_map = {}
                # HARD RETURN: stop further legacy allocation/recalculation logic
                print("🚨 IQF FULL ACCEPT - returning early to prevent downstream recalculation")
                return Response({
                    "success": True,
                    "trays": all_trays,
                ...
                # Default: use accepted trays from store
...
class IQFPickCompleteTableTrayIdListAPIView(APIView):
    def get(self, request):
        ...
        try:
            print(f"🔍 [IQFPickCompleteTableTrayIdListAPIView] STRICT Brass QC rejection mode for lot_id: {lot_id}")
            from Brass_QC.models import Brass_QC_Rejected_TrayScan
            from modelmasterapp.models import TrayId
            from Brass_QC.models import Brass_QC_Rejection_ReasonStore

            # Step 1: Get all rejected tray IDs from Brass QC rejection scan (may have duplicates per reason)
            brass_qc_rejected_trays = Brass_QC_Rejected_TrayScan.objects.filter(
                lot_id=lot_id,
                rejected_tray_id__isnull=False
            ).exclude(rejected_tray_id='')
            rejected_tray_ids = [t.rejected_tray_id for t in brass_qc_rejected_trays]
            print(f"   [IQF] Brass QC rejected tray IDs: {rejected_tray_ids}")

            # Step 2: Remove duplicates (CRITICAL)
            unique_tray_ids = list(set(rejected_tray_ids))
            print(f"   [IQF] Unique tray IDs: {unique_tray_ids}")

            # Step 3: Fetch original qty from TrayId for each unique rejected tray
            tray_records = TrayId.objects.filter(
                lot_id=lot_id,
                tray_id__in=unique_tray_ids
            )
            tray_map = {t.tray_id: t.tray_quantity for t in tray_records}
            print(f"   [IQF] TrayId quantities: {tray_map}")

            # Step 4: Build result list (NO duplicates)
            result = []
            seen = set()
            for tray_id in unique_tray_ids:
                if tray_id in seen:
                    continue
                qty = tray_map.get(tray_id, 0)
                result.append({
                    "tray_id": tray_id,
                    "qty": qty,
                    "is_top": False
                ...
                seen.add(tray_id)
            print(f"   [IQF] Result tray list: {result}")

            # Step 5: Validation - total qty must match Brass QC rejection qty
            total_tray_qty = sum(t["qty"] for t in result)
            rejection_store = Brass_QC_Rejection_ReasonStore.objects.filter(lot_id=lot_id).order_by('-created_at').first()
            total_rejection_qty = rejection_store.total_rejection_quantity if rejection_store else None
            print(f"   [IQF] Total tray qty: {total_tray_qty}, Brass QC rejection qty: {total_rejection_qty}")
            if total_rejection_qty is not None and total_tray_qty != total_rejection_qty:
                raise Exception(f"❌ IQF Tray Qty mismatch with Brass QC rejection: {total_tray_qty} != {total_rejection_qty}")

            return Response({
                "success": True,
                "trays": result,
                "total_trays": len(result)
            })
...
class IQFTrayRejectionAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Step 1a: Zero rejection → STRICT PASS-THROUGH
            # MUST: treat IQF incoming trays as authoritative and persist them unchanged.
            if total_rejection_qty == 0:
                stock = TotalStockModel.objects.filter(lot_id=lot_id).first()

                # Build accepted map from ORIGINAL BrassTrayId records
                # IQFTrayId may contain split/processed data; use BrassTrayId as source of truth.
                accepted_qty_map = {}
                iqf_tray_ids = IQFTrayId.objects.filter(lot_id=lot_id).values_list('tray_id', flat=True)
                brass_trays = BrassTrayId.objects.filter(lot_id=lot_id, tray_id__in=list(iqf_tray_ids)).exclude(tray_quantity__lte=0)
                if not brass_trays.exists():
                    return Response({
                        'success': False,
                        'error': f'No original Brass tray records found for lot {lot_id}'
                    }, status=400)

                for bt in brass_trays:
                    if bt.tray_id:
                        accepted_qty_map[bt.tray_id] = accepted_qty_map.get(bt.tray_id, 0) + int(bt.tray_quantity or 0)

                # Remove previous rejection/accepted draft/state for this lot
                IQF_Rejected_TrayScan.objects.filter(lot_id=lot_id).delete()
                IQF_Accepted_TrayID_Store.objects.filter(lot_id=lot_id).delete()

                # Persist exact IQF trays as accepted (no redistribution, no min(), no split)
                for tray_id, tray_qty in accepted_qty_map.items():
                    ...
                    print(f"   ✅ Zero rejection - persisted IQF tray: {tray_id} qty={tray_qty}")

                total_accepted_qty = sum(accepted_qty_map.values())

                # Update lot-level accepted qty for Brass QC and IQF bookkeeping
                if stock:
                    # Primary field requested by user
                    if hasattr(stock, 'brass_qc_accepted_qty'):
                        stock.brass_qc_accepted_qty = total_accepted_qty
                    # Keep IQF flags consistent
                    ...
                    if hasattr(stock, 'brass_qc_accepted_qty'):
                        update_fields.insert(0, 'brass_qc_accepted_qty')
                    stock.save(update_fields=update_fields)
                    print(f"   ✅ Zero rejection acceptance completed for lot {lot_id}: qty={total_accepted_qty}")

                # Mark request as handled for zero-rejection to guard against
                # any accidental fall-through into legacy allocation logic.
                try:
                    request._zero_rejection_handled = True
                except Exception:
                    pass

                # Clean any drafts and return success
                IQF_Draft_Store.objects.filter(
                    lot_id=lot_id,
                ...
                return Response({
                    'success': True,
                    'message': 'Current IQF input trays accepted as-is (no rejection)',
                    'allocation_type': 'direct_iqf_pass_through',
                    'no_verification_required': True,
                    'total_accepted': total_accepted_qty
                })

            # Defensive guard: if zero-rejection was handled above but execution
            # somehow continues, stop here immediately.
            if getattr(request, '_zero_rejection_handled', False):
                print("🔥 SHOULD NOT REACH BELOW THIS LINE - zero-rejection already handled. Exiting now.")
                return Response({
                    'success': True,
                    'message': 'Zero rejection already handled (guard exit)'
                })

            # Step 2: Get original available trays BEFORE saving frontend accepted trays
            # Use universal helper to find ALL lot trays from every source
            accepted_tray_ids = []  # not saved yet
            stock = TotalStockModel.objects.filter(lot_id=lot_id).first()
            use_audit = bool(getattr(stock, 'send_brass_audit_to_iqf', False))

            # Universal tray discovery — finds trays regardless of lot source
            all_lot_trays_map = _get_all_lot_trays_for_iqf(lot_id)
            eligible_tray_ids = set(all_lot_trays_map.keys())
            print(f"📦 [ELIGIBLE] All lot tray IDs from universal helper: {eligible_tray_ids}")
            ...
            def _best_qty_for_tray(tid):
                # Prefer qty from universal helper (already max across all sources)
                if tid in all_lot_trays_map:
                    return all_lot_trays_map[tid]
                qtys = []
...
def iqf_get_rejected_tray_scan_data(request):
    ...
    try:
        ...
        # regenerate the correct distribution from ALL lot trays (universal helper).
        if total_rejection_qty > 0 and tray_capacity > 0 and total_rejection_qty > tray_capacity:
            ...
            if sum_rej_qty != total_rejection_qty:
                ...
                print(f"[iqf_get_rejected_tray_scan_data] Regenerating from all lot trays...")

                accepted_ids_set = set(
                    IQF_Accepted_TrayID_Store.objects.filter(lot_id=lot_id).values_list('tray_id', flat=True)
                )
                # Use universal helper to find ALL lot trays
                all_lot_trays_regen = _get_all_lot_trays_for_iqf(lot_id)
                eligible = [tid for tid in sorted(all_lot_trays_regen.keys()) if tid not in accepted_ids_set]
...
def iqf_get_delink_candidates(request):
    ...
    try:
        # 1. Get ALL trays for this lot from every relevant source.
        #    Uses the universal helper so Brass QC, Brass Audit, and direct IQF
        #    lots all find their complete tray set.
        all_trays_dict = _get_all_lot_trays_for_iqf(lot_id)
...
def iqf_get_remaining_trays(request):
    ...
    try:
        ...
        # 1) Build IQF-eligible tray IDs from ALL sources using universal helper.
        #    This fixes the bug where lots from Brass Audit/QC had incomplete tray
        #    discovery because of source-specific fallback cascades.
        all_lot_trays = _get_all_lot_trays_for_iqf(lot_id)
        eligible_tray_ids = set(all_lot_trays.keys())
        print(f"[DEBUG] IQF-eligible tray IDs: {eligible_tray_ids}")

        def _best_qty_for_tray(tid):
            # Prefer the qty from the universal helper (already max across sources)
            if tid in all_lot_trays:
                return all_lot_trays[tid]
            qtys = []
            for model in [IQFTrayId, DPTrayId_History, BrassTrayId, BrassAuditTrayId, IPTrayId]:
                try:
                    q = model.objects.filter(lot_id=lot_id, tray_id=tid).values_list('tray_quantity', flat=True).first()
        ...
        if eligible_tray_ids:
            found_iqf_trays = list(IQFTrayId.objects.filter(tray_id__in=eligible_tray_ids, lot_id=lot_id).distinct().order_by('id'))
...
def get_tray_capacity(request):
    ...
    try:
        ...
        if tray_capacity and tray_capacity > 0:
            # Use universal helper to find ALL lot trays
            all_lot_trays_cap = _get_all_lot_trays_for_iqf(lot_id)
            total_tray_count = len(all_lot_trays_cap)

            # Fallback: estimate tray count from physical qty if no trays found
...
def get_iqf_available_trays_for_allocation(lot_id):
    ...
    try:
        ...
        if trays.exists():
            ...
            # Build tray list from eligible IDs using qty from universal helper
            tray_list = []
            for tray_id in eligible_tray_ids:
                qty = all_lot_trays_map.get(tray_id, 0)
                if qty > 0:
                    master_tray = TrayId.objects.filter(tray_id=tray_id).first()
                    raw_capacity = None
                    if master_tray:
                    ...
                    if not raw_capacity and _stock_tray_capacity:
                        raw_capacity = _stock_tray_capacity
                    if not raw_capacity:
                        raw_capacity = qty
                    tray_data = {
                        'tray_id': tray_id,
