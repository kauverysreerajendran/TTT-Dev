...
class SaveRejectionDraftAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Validate and filter tray_scans to only include valid tray IDs
            valid_tray_scans = []
            for tray_scan in tray_scans:
                tray_id = tray_scan.get('tray_id', '').strip()
                if not tray_id:
                    continue
                try:
                    tray_obj = TrayId.objects.get(tray_id=tray_id)
                    # Validate tray type compatibility
                    validation_result = validate_tray_type_compatibility(tray_obj, lot_id)
                    if not validation_result['is_compatible']:
                        continue  # Skip invalid tray
                    # Check if tray is already used in this lot
                    existing_usage = IPTrayId.objects.filter(tray_id=tray_id, lot_id=lot_id).first()
                    if existing_usage:
                        continue  # Skip already used tray
                    valid_tray_scans.append(tray_scan)
                except TrayId.DoesNotExist:
                    continue  # Skip non-existent tray

            # Prepare draft data with only valid tray_scans
            draft_data = {
                'batch_id': batch_id,
