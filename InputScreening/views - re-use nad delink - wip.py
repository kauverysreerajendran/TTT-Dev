...
class TrayRejectionAPIView(APIView):
    ...
    def get_actual_tray_distribution(self, lot_id, tray_capacity):
        ...
        try:
            ...
            tray_records = IPTrayId.objects.filter(lot_id=lot_id).order_by('date')
    ...
    def post(self, request):
        try:
            ...
            # ✅ NEW: DELINK VALIDATION - Check if delink is required before processing
            delink_trays = data.get('delink_trays', [])  # Get delink tray data from request
            
            # Calculate delink requirements
            try:
                original_distribution = get_actual_tray_distribution_for_delink(lot_id, total_stock_obj)
                print(f"🔍 [DELINK_CHECK] Original distribution: {original_distribution}")
                
                # Count existing trays being reused (not new trays)
                reused_existing_trays = 0
                for scan in tray_scans:
                    tray_id = scan.get('tray_id', '').strip()
                    if tray_id:
                        # Check if this is an existing tray (not new)
                        tray_obj = TrayId.objects.filter(tray_id=tray_id).first()
                        is_new = getattr(tray_obj, 'new_tray', False) and not tray_obj.lot_id if tray_obj else False
                        
                        if not is_new:
                            # This is an existing tray being reused
                            reused_existing_trays += 1
                            print(f"🔍 [DELINK_CHECK] Found existing tray reuse: {tray_id}")
                
                # Calculate how many trays will have 0 qty after rejections
                ...
                if total_stock_obj.batch_id and hasattr(total_stock_obj.batch_id, 'tray_capacity'):
                    tray_capacity = total_stock_obj.batch_id.tray_capacity
                
                # Calculate expected distribution after rejections
                if remaining_qty > 0:
                    trays_needed = math.ceil(remaining_qty / tray_capacity)
                    zero_trays = len(original_distribution) - trays_needed
                else:
                    zero_trays = len(original_distribution)
                
                # Delink requirement calculation (cause-based)
                required_delinks = min(zero_trays, reused_existing_trays)
                
                print(f"🔍 [DELINK_CHECK] Zero trays after rejection: {zero_trays}")
                print(f"🔍 [DELINK_CHECK] Reused existing trays: {reused_existing_trays}")
                print(f"🔍 [DELINK_CHECK] Required delinks: {required_delinks}")
                
                # Check if delinks are required but not provided
                if required_delinks > 0:
                    has_delink_trays = bool(delink_trays and any(d.get('tray_id') for d in delink_trays))
                    
                    if not has_delink_trays:
                        # ✅ FIX: Do NOT block rejection when delinks are missing.
                        # Delinks are handled in the accepted tray modal (showAcceptedTrayModal)
                        # via loadDelinkTrayData / get_delink_tray_data API after rejection is saved.
                        # The frontend never sends delink_trays in the initial POST.
                        print(f"⚠️ [DELINK_CHECK] {required_delinks} delink(s) required but not provided. "
                              f"Will be handled in accepted tray modal via get_delink_tray_data API.")
                    
                    # Validate provided delinks if they ARE included
                    elif has_delink_trays:
                        provided_delinks = sum(1 for d in delink_trays if d.get('tray_id'))
                        if provided_delinks < required_delinks:
                            return Response({
                                'success': False,
                                'error': f'Insufficient delinks provided. Required: {required_delinks}, Provided: {provided_delinks}',
                                'requires_delink': True,
                                'required_delinks': required_delinks
                            }, status=400)
                
                print(f"✅ [DELINK_CHECK] Validation passed. Required: {required_delinks}, Has delinks: {bool(delink_trays)}")
                
            except Exception as delink_error:
                print(f"⚠️ [DELINK_CHECK] Error during delink validation: {delink_error}")
                # Continue processing if delink validation fails (non-blocking)
                
            # END DELINK VALIDATION
            ...
            # ✅ NEW: Process delink trays if provided
            if delink_trays:
                for delink in delink_trays:
                    delink_tray_id = delink.get('tray_id', '').strip()
                    if delink_tray_id:
                        try:
                            # Mark tray as delinked in TrayId table
                            tray_obj = TrayId.objects.filter(tray_id=delink_tray_id).first()
                            if tray_obj:
                                tray_obj.delink_tray = True
                                tray_obj.save(update_fields=['delink_tray'])
                                print(f"✅ Marked tray {delink_tray_id} as delinked")
                            else:
                                print(f"⚠️ Delink tray {delink_tray_id} not found in TrayId table")
                                
                            # Also mark in IPTrayId table if exists
                            ip_tray_obj = IPTrayId.objects.filter(tray_id=delink_tray_id, lot_id=lot_id).first()
                            if ip_tray_obj:
                                ip_tray_obj.delink_tray = True
                                ip_tray_obj.save(update_fields=['delink_tray'])
                                print(f"✅ Marked IPTrayId {delink_tray_id} as delinked")
                                
                        except Exception as delink_err:
                            print(f"❌ Error processing delink tray {delink_tray_id}: {delink_err}")
                            continue
                            
                print(f"✅ Processed {len([d for d in delink_trays if d.get('tray_id')])} delink trays")
...
def get_accepted_tray_scan_data(request):
    ...
    try:
        ...
        if top_tray_qty == 0 and available_qty > 0:
            ...
            top_tray_qty = remainder if remainder > 0 else tray_capacity

        # -------------------------------
        # Delink logic (CAUSE-BASED)
        # -------------------------------
        zero_trays = current_distribution.count(0)
        reused_existing_trays = len(reused_tray_indices)
        ...
        if has_draft:
            ...
            stored_delinks = draft_record.delink_trays or []

            # 🔒 Clamp delinks to physically justified count
            delink_trays = stored_delinks[:required_delinks]
        ...
        return Response({
            ...
            'draft_tray_id': draft_record.top_tray_id if draft_record else "",
            'top_tray_verified': top_tray_verified,
            'verified_tray_qty': verified_tray_qty,
            'debug_info': {
                'original_distribution': original_distribution,
                'current_distribution': current_distribution,
                'zero_trays': zero_trays,
                'reused_existing_trays': reused_existing_trays,
                'required_delinks': required_delinks
            }
        })
...
class IS_Completed_Table(APIView):
    ...
    def get(self, request):
        ...
        for total_stock_obj in page_obj.object_list:
            ...
            actual_trays = IPTrayId.objects.filter(
                lot_id=data['stock_lot_id'],
                delink_tray=False,
                rejected_tray=False,
                tray_quantity__gt=0
            )

            # ✅ FIX: Accepted quantity from actual trays (same source as no_of_trays)
            accepted_qty_from_trays = actual_trays.aggregate(
                total=Sum('tray_quantity')
            )['total'] or 0

            data['display_accepted_qty'] = accepted_qty_from_trays
...
class IS_AcceptTable(APIView):
    ...
    def get(self, request):
        ...
        for total_stock_obj in page_obj.object_list:
            ...
            total_stock = total_stock_obj.total_stock or 0
            # Calculate no_of_trays
            # FIX: derive tray count from actual trays, not math
            lot_id = data['stock_lot_id']
            ...
            data['no_of_trays'] = actual_trays.count()
...
class IS_RejectTable(APIView):
    ...
    def get(self, request):
        ...
        for total_stock_obj in page_obj.object_list:
            ...
            active_tray_count = IPTrayId.objects.filter(
                lot_id=stock_lot_id,
                delink_tray=False
            ).count()
...
def get_delink_tray_data(request):
    """
    Delink logic:
    - Z = number of empty trays
    - R = number of reused trays
    - Delink only (Z - R) trays
    - Reused trays cancel emptiness first
    """
    ...
    try:
        ...
        print(f"🔍 [get_delink_tray_data] Starting at {start_time.isoformat()} for lot_id: {lot_id}")

        stock = TotalStockModel.objects.filter(lot_id=lot_id).first()
        if not stock:
            return Response({'success': False, 'error': 'No stock record found'}, status=404)

        original_distribution = get_actual_tray_distribution_for_delink(lot_id, stock)

        # Try to get draft rejections first if no committed rejections exist
        draft_rejections = None
        committed_rejections = IP_Rejected_TrayScan.objects.filter(lot_id=lot_id)
        
        print(f"🔍 [get_delink_tray_data] Found {committed_rejections.count()} committed rejections")
        
        if not committed_rejections.exists():
            print(f"🔍 [get_delink_tray_data] No committed rejections found, checking for draft data")
            try:
                draft = IP_Rejection_Draft.objects.filter(lot_id=lot_id).first()
                if draft and hasattr(draft, 'draft_data'):
                    if isinstance(draft.draft_data, str):
                        draft_data = json.loads(draft.draft_data)
                    else:
                        draft_data = draft.draft_data
                    
                    # Convert draft tray_scans to rejection format
                    tray_scans = draft_data.get('tray_scans', [])
                    draft_rejections = []
                    for scan in tray_scans:
                        if isinstance(scan, dict):
                            ...
                            tray_id = scan
                            qty = 0  # Will be determined from rejection_data
                        
                        # Find qty from rejection_data if not in tray_scan
                        if qty == 0:
                            for rej_data in draft_data.get('rejection_data', []):
                                tray_ids = rej_data.get('tray_ids', [])
                                if isinstance(tray_ids, str):
                                    tray_ids = [tray_ids]
                                if tray_id in tray_ids:
                                    qty = rej_data.get('qty', 0)
                                    break
                        
                        if tray_id and qty > 0:
                            draft_rejections.append({
                                'tray_id': tray_id,
                                'qty': qty
                            })
                    
                    print(f"✅ [get_delink_tray_data] Found draft rejections: {len(draft_rejections)} items")
            except Exception as draft_error:
                print(f"❌ [get_delink_tray_data] Error loading draft data: {draft_error}")
        ...
        (
            ...
            lot_id, original_distribution, draft_rejections
        )

        # ---------------------------------------
        # Compute counts
        # ✅ FIX: Use current_distribution (normalized) instead of raw_distribution
        # raw_distribution only reflects direct tray manipulations during rejection,
        # NOT the final redistributed layout. After normalization, items are packed
        # into bottom trays, making top trays empty — those are the real zeros.
        # Example: raw=[12,16,4,0] has Z=1, but normalized=[0,0,16,16] has Z=2
        # ---------------------------------------
        ...
        # ---------------------------------------
        # Choose trays to delink
        # Prefer trays NOT involved in reuse
        # ---------------------------------------
        candidate_indices = [
            i for i in zero_indices
            if i not in reused_tray_indices
        ]
        ...
        # ---------------------------------------
        delink_data = []
        for idx, tray_index in enumerate(delink_indices):
            original_qty = (
                original_distribution[tray_index]
                if tray_index < len(original_distribution)
                else 0
            )
            ...
            delink_data.append({
                ...
                'source': 'unexplained_empty_after_normalization',
                'previous_tray_quantity': original_qty
            })


        print(
            f"[DELINK] normalized={current_distribution}, "
            f"raw={raw_distribution}, "
            f"zero_indices={zero_indices}, "
            f"reused_indices={reused_tray_indices}, "
            f"delink_indices={delink_indices}"
        )

        # ✅ Add final timing and result debug
        end_time = datetime.now()
        processing_time = (end_time - start_time).total_seconds() * 1000
        print(f"🔍 [get_delink_tray_data] Completed in {processing_time:.1f}ms, returning {len(delink_data)} delink trays")
        ...
        return Response({
            ...
            'lot_id': lot_id,
            'debug_info': {
                'normalized_distribution': current_distribution,
                'raw_distribution': raw_distribution,
                'original_distribution': original_distribution,
                'zero_count': Z,
                'reused_count': R,
                'delink_count': delink_count,
                'reused_tray_indices': list(reused_tray_indices),
                'delink_indices': delink_indices,
...
def calculate_distribution_after_rejections(lot_id, original_distribution, draft_rejections=None):
    """
    Calculate tray distribution after rejections.

    Args:
        lot_id: The lot ID
        original_distribution: Original tray distribution
        draft_rejections: Optional draft rejection data to use instead of database rejections

    Guarantees:
    - reused_tray_indices contains ONLY indices of EXISTING trays reused
    - indices always refer to original_distribution positions
    """

    current_distribution = original_distribution.copy()
    floating_extra_qty = 0

    # ✅ CLEAN invariant: indices ONLY from original_distribution
    reused_tray_indices = set()

    tray_count = len(original_distribution)

    # ✅ Get ip_trays in same order as original_distribution
    ip_trays = list(IPTrayId.objects.filter(lot_id=lot_id).order_by('date'))

    # Use draft rejections if provided, otherwise query database
    if draft_rejections:
        rejections = draft_rejections
        print(f"DEBUG: Using draft rejections: {len(rejections)} items")
    else:
        rejections = IP_Rejected_TrayScan.objects.filter(
            lot_id=lot_id
        ).order_by('id')
        print(f"DEBUG: Using database rejections: {rejections.count()} items")

    print(f"DEBUG: Starting distribution: {original_distribution}")

    for rejection in rejections:
        # Handle both database objects and draft data
        if hasattr(rejection, 'rejected_tray_quantity'):
            # Database object
            rejected_qty = int(rejection.rejected_tray_quantity or 0)
            tray_id = (rejection.rejected_tray_id or "").strip()
        else:
            # Draft data format
            rejected_qty = int(rejection.get('qty', 0))
            tray_id = rejection.get('tray_id', '').strip()

        if rejected_qty <= 0:
            continue

        # -------------------------------
        # Scenario A: SHORTAGE (no tray)
        # -------------------------------
        if not tray_id:
            ...
            if rejected_qty > 0:
                current_distribution = consume_shortage_from_distribution(
                    current_distribution, rejected_qty
                )
            continue

        # -------------------------------
        # Scenario B: Tray-based rejection
        # -------------------------------
        is_new = is_new_tray_by_id(tray_id)

        if is_new:
            # ✅ New trays NEVER cause delink
            if floating_extra_qty > 0:
                consumed = min(floating_extra_qty, rejected_qty)
                floating_extra_qty -= consumed
                rejected_qty -= consumed
            ...
            if rejected_qty > 0:
                current_distribution = free_up_space_optimally(
                    current_distribution, rejected_qty
                )
        else:
            # ❌ EXISTING tray reused → THIS causes delink
            # Find the index of this tray in ip_trays
            tray_index = None
            for idx, ip_tray in enumerate(ip_trays):
                if ip_tray.tray_id == tray_id:
                    tray_index = idx
                    break
            
            if tray_index is not None and 0 <= tray_index < len(current_distribution):
                # Zero the specific tray
                freed_good_items = current_distribution[tray_index]
                current_distribution[tray_index] = 0
                floating_extra_qty += freed_good_items
                reused_tray_indices.add(tray_index)
                print(f"DEBUG: Zeroed tray at index {tray_index}, freed {freed_good_items} items")
            else:
                print(f"⚠️ Could not find tray {tray_id} in ip_trays or invalid index {tray_index}")
                # Fallback: use old logic
                (
                    current_distribution,
                    freed_good_items,
                    zeroed_index
                ) = remove_rejected_tray_from_distribution(
                    current_distribution,
                    rejected_qty
                )
                if zeroed_index is not None and 0 <= zeroed_index < tray_count:
                    reused_tray_indices.add(zeroed_index)
                floating_extra_qty += freed_good_items

        print(f"DEBUG: Distribution step: {current_distribution}")

    # -------------------------------
    # Final floating merge (NO reuse)
    # -------------------------------
    if floating_extra_qty > 0:
    ...
    raw_distribution = current_distribution.copy()

    # -------------------------------
    # Normalization (structure-safe)
    # -------------------------------
...
def normalize_distribution(total_qty, tray_capacity, tray_count):
    """
    Normalize quantity while PRESERVING tray count.

    Rules:
    - Sum(distribution) == total_qty
    - Each tray <= tray_capacity
    - distribution length == tray_count
    - Zero-quantity trays are preserved
    """

    if tray_count <= 0:
        return []

    distribution = [0] * tray_count
...
def remove_rejected_tray_from_distribution(distribution, rejected_qty):
    ...
    Returns: (updated_distribution, freed_good_items, zeroed_index)
    
    CRITICAL: Sets tray quantity to 0 instead of removing it.
    This preserves physical tray identity so delink can detect zeros.
    
    freed_good_items = (tray quantity) - (rejected_qty)
    These are valid items displaced from the tray.
    
    Prefers non-top-tray (index > 0) to preserve top tray identity.
    """
    result = distribution.copy()
    total_available = sum(result)
    
    if total_available < rejected_qty:
        return result, 0, None  # Should not happen ideally
    
    # Step 1: Try to find exact match, prefer non-top-tray (index > 0)
    for i in range(1, len(result)):
        if result[i] == rejected_qty:
            result[i] = 0  # Zero out, do NOT pop
            return result, 0, i  # No freed good items, index i used
    # Fallback: check top tray
    if len(result) > 0 and result[0] == rejected_qty:
        if len(result) > 0 and result[0] == rejected_qty:
            result[0] = 0
        return result, 0, 0
    
    # Step 2: No exact match. Find best tray (minimum waste).
    best_idx = -1
    min_waste = float('inf')
    
    # Prefer non-top trays (index > 0)
    for i in range(1, len(result)):
        qty = result[i]
        if qty >= rejected_qty:
    ...
    # Fallback to top tray
    if best_idx == -1 and len(result) > 0 and result[0] >= rejected_qty:
        best_idx = 0
    
    if best_idx != -1:
        original_qty = result[best_idx]
        freed_good_items = original_qty - rejected_qty
        result[best_idx] = 0  # Zero out, do NOT pop
        result[best_idx] = 0  # Zero out, do NOT pop
        print(f"DEBUG: Zeroed tray at index {best_idx} (was {original_qty}), freed {freed_good_items} good items")
        return result, freed_good_items, best_idx

    # Fallback: zero out the largest non-top tray
    if len(result) > 1:
    ...
    original_qty = result[max_idx]
    freed_good_items = max(0, original_qty - rejected_qty)
    result[max_idx] = 0  # Zero out, do NOT pop
    return result, freed_good_items, max_idx
...
def free_up_space_optimally(distribution, qty_to_free):
    ...
    # Do NOT remove zero-quantity trays — zeros must be preserved for delink detection
...
def get_actual_tray_distribution_for_delink(lot_id, stock):
    ...
    try:
        ...
        # Option 1: Get from IPTrayId model first (same as get_actual_tray_distribution)
        try:
            tray_records = IPTrayId.objects.filter(lot_id=lot_id).order_by('date')
            if tray_records.exists():
                tray_quantities = []
                for tray in tray_records:
                    if hasattr(tray, 'tray_quantity') and tray.tray_quantity:
                        tray_quantities.append(tray.tray_quantity)
                        print(f"🔍 IPTrayId {tray.tray_id}: quantity = {tray.tray_quantity}")
                
                if tray_quantities:
                    print(f"✅ Found actual tray distribution from IPTrayId: {tray_quantities}")
                    return tray_quantities
        except Exception as e:
            print(f"Could not get tray distribution from IPTrayId: {e}")
        
        # Option 2: Calculate from TotalStockModel (same as get_actual_tray_distribution)
        print(f"🔍 Trying Method 1: IPTrayId records")
        tray_records = IPTrayId.objects.filter(lot_id=lot_id).order_by('date')
        if tray_records.exists():
            print(f"🔍 Found {tray_records.count()} IPTrayId records")
            tray_quantities = []
            for tray in tray_records:
                if hasattr(tray, 'tray_quantity') and tray.tray_quantity:
                    tray_quantities.append(tray.tray_quantity)
        ...
        print(f"⚠️ ALL METHODS FAILED: Using minimal fallback distribution with tray_capacity=10")
        return [10]  # At least one tray with standard capacity
    ...
    def get(self, request):
        ...
        for hist in delinked_history:
            ...
            delinked_trays_list.append({
                ...
                'tray_quantity': 0, # Handle potential None
        ...
        if accepted_ip_stock:
            # ACCEPT TABLE: accepted trays ONLY
            trays_to_process = list(accepted_trays)
            print("Accept Table => accepted trays only")

        elif rejected_ip_stock:
            # Reject table → rejected + delinked
            trays_to_process = list(rejected_trays) + delinked_trays_list
            print("Reject Table => rejected + delinked trays")

        elif few_cases_accepted_ip_stock:
            # Complete table → full picture
            trays_to_process = (
                list(accepted_trays)
                + list(rejected_trays)
                + delinked_trays_list
            )
            print("Complete Table => accepted + rejected + delinked")

        else:
            trays_to_process = list(active_trays)
        ...
        for tray in trays_to_process:
            ...
            if ifattrs_top and not attrs.get('delink_tray') and not top_tray_data:
                top_tray_data = attrs
            else:
    ...
    try:
        ...
        if remaining_reuse_slots <= 0:
            return JsonResponse({
                ...
                'status_message': 'Reuse Limit Reached. Scan new tray'
            })
