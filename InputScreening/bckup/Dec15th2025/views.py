...
def reject_check_tray_id_simple(request):
    ...
    # --- DRAFT MODE LOGIC ---
    if is_draft:
        # Only use current_session_allocations for validation, ignore DB rejections
        available_tray_quantities, _ = get_available_quantities_with_session_allocations(
            current_lot_id, current_session_allocations
        )
        tray_list = list(IPTrayId.objects.filter(lot_id=current_lot_id, rejected_tray=False).order_by('date'))
        tray_index = None
        for idx, t in enumerate(tray_list):
        ...
        remaining_qty = rejection_qty
        for i in range(len(simulated_quantities)):
            if remaining_qty <= 0:
                break
        ...
        total_trays = len(simulated_quantities)
        if rejection_qty > 0 and rejection_qty < total_trays:
            return JsonResponse({
                'exists': True,
                'valid_for_rejection': True,
                'status_message': 'Tray can re-use',
                'validation_type': 'existing_reusable_qty_lt_tray_count_draft',
                'available_quantities': simulated_quantities,
                'total_current': sum(simulated_quantities),
                'tray_type_compatible': True
            })

        zero_qty_indices = [i for i, qty in enumerate(simulated_quantities) if qty == 0]
        can_reuse = tray_index is not None and len(zero_qty_indices) > 0
        ...
        if can_reuse:
            return JsonResponse({
                ...
                'valid_for_rejection': True,
                'status_message': 'Tray can re-use',
                'validation_type': 'existing_reusable_draft',
                'available_quantities': simulated_quantities,
                'total_current': sum(simulated_quantities),
                'tray_type_compatible': True
            })
            ...
            return JsonResponse({
                'exists': True,
                'valid_for_rejection': False,
                'error': 'Reuse restricted: No tray with 0 qty available for rejection',
                'status_message': 'Reuse restricted',
                'validation_type': 'reuse_restricted_draft',
                'available_quantities': simulated_quantities,
                'total_current': sum(simulated_quantities)
            })
    ...
    if tray_id in used_trays_for_other_reasons:
        tray_capacity = getattr(tray_obj, 'tray_capacity', 12)
        tray_qty_used = sum(
            int(alloc.get('qty', 0))
            for alloc in current_session_allocations
            if alloc.get('tray_id') == tray_id
        )
        if tray_qty_used >= tray_capacity:
            return JsonResponse({
                'exists': True,
                'valid_for_rejection': False,
                'error': 'Tray already used for another rejection reason and capacity is exhausted',
                'status_message': 'Tray capacity exhausted'
            })
    ...
    if rejection_qty > 0 and rejection_qty < total_trays:
        return JsonResponse({
            ...
            'valid_for_rejection': True,
            'status_message': 'Tray can re-use',
            'validation_type': 'existing_reusable_qty_lt_tray_count',
            'available_quantities': simulated_quantities,
            'total_current': sum(simulated_quantities),
            'tray_type_compatible': True
        })
    ...
    if can_reuse:
        ...
        return JsonResponse({
            ...
            'error': 'Reuse restricted: No tray with 0 qty available for rejection',
            'status_message': 'Reuse restricted',
            'validation_type': 'reuse_restricted',
            'available_quantities': simulated_quantities,
            'total_current': sum(simulated_quantities)
        })
...
class SaveRejectionDraftAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # ✅ FIX: Update BOTH ip_onhold_picking AND draft_tray_verify to True
            try:
                total_stock = TotalStockModel.objects.filter(lot_id=lot_id).first()
                if total_stock:
                    total_stock.ip_onhold_picking = True
                    total_stock.draft_tray_verify = True  # ✅ ADD THIS LINE
