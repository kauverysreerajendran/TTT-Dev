...
def get_tray_verification_status(request):
    ...
    try:
        ...
        for record in verification_records:
            verification_data[record.tray_id] = {
...
def reset_tray_verification_for_lot(request):
    ...
    try:
        ...
        if tray_ids:
            ...
            IPTrayId.objects.filter(tray_id__in=tray_ids, lot_id=lot_id).update(IP_tray_verified=False)
...
def validate_tray_reuse_for_rejection(original_distribution, rejection_qty, tray_ids_used):
    """
    Enforce that only as many unique trays can be reused as trays emptied by the rejection qty.
    - original_distribution: list of ints, e.g. [2, 12, 12, 12, 12]
    - rejection_qty: int, total rejection qty for the reason
    - tray_ids_used: list of tray_id strings already used for this reason (including the current one being validated)
    Returns: (is_valid, allowed_tray_count, trays_emptied_indices, error_message)
    """
    # Defensive copy
    tray_distribution = list(original_distribution)
    trays_emptied = []
    remaining = rejection_qty

    # Go through trays in order (as per business rule)
    for idx, qty in enumerate(tray_distribution):
        if remaining <= 0:
            break
        if remaining >= qty:
            trays_emptied.append(idx)
            remaining -= qty
        else:
            break

    allowed_tray_count = len(trays_emptied)
    unique_trays_used = set(tray_ids_used)
    is_valid = len(unique_trays_used) <= allowed_tray_count

    error_message = ""
    if not is_valid:
        error_message = (
            f"Only {allowed_tray_count} unique tray(s) can be reused for this rejection qty ({rejection_qty}). "
            f"You have used {len(unique_trays_used)}: {list(unique_trays_used)}. "
            f"Trays emptied: {trays_emptied} (distribution: {original_distribution})"
        )

    return is_valid, allowed_tray_count, trays_emptied, error_message


def get_trays_used_for_rejection(original_distribution, rejection_qty):
    """
    Returns the indices of trays (in order) that are used (fully or partially) to fulfill the rejection_qty.
    """
    trays_used = []
    remaining = rejection_qty
    for idx, qty in enumerate(original_distribution):
        if remaining <= 0:
            break
        trays_used.append(idx)
        if remaining >= qty:
            remaining -= qty
        else:
            # Last tray is only partially used
            remaining = 0
    return trays_used

def validate_tray_reuse_for_rejection_v2(original_distribution, rejection_qty, tray_ids_used):
    """
    Allow reuse of as many unique trays as are used (fully or partially) to fulfill the rejection_qty.
    """
    trays_used = get_trays_used_for_rejection(original_distribution, rejection_qty)
    allowed_tray_count = len(trays_used)
    unique_trays_used = set(tray_ids_used)
    is_valid = len(unique_trays_used) <= allowed_tray_count
    ...
    if not is_valid:
        error_message = (
            f"Only {allowed_tray_count} unique tray(s) can be reused for this rejection qty ({rejection_qty}). "
            f"You have used {len(unique_trays_used)}: {list(unique_trays_used)}. "
            f"Trays used: {trays_used} (distribution: {original_distribution})"
        )

    return is_valid, allowed_tray_count, trays_used, error_message

# Example usage:
# original_distribution = [2, 12, 12, 12, 12]
# rejection_qty = 13
# tray_ids_used = ['JB-A00025', 'JB-A00024']
# is_valid, allowed, used, msg = validate_tray_reuse_for_rejection_v2(original_distribution, rejection_qty, tray_ids_used)
# print(is_valid, allowed, used, msg)