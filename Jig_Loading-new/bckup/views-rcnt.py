...
def jig_autosave(request):
    try:
        ...
        try:
            prev_draft = JigAutoSave.objects.get(user=request.user, lot_id=lot_id)
            prev_exists = True
        except JigAutoSave.DoesNotExist:
            prev_exists = False

        # FIXED: More lenient empty check - only consider empty if ALL key fields are empty
        jig_qr_filled = bool(data.get('jig_qr_id', '').strip())
        has_tray_data = bool(data.get('delink_tray_data', []) or data.get('half_filled_tray_data', []))
        has_quantities = bool(data.get('total_cases_loaded', 0) > 0)
        has_cycle_data = data.get('no_of_cycle_count', '-/-') not in ('', '-/-', None)
        
        # CRITICAL FIX: Don't consider it empty if JIG QR ID is filled
        is_empty = not (jig_qr_filled or has_tray_data or has_quantities or has_cycle_data)

        if prev_exists and is_empty:
            print(f"🚫 Not overwriting existing auto-save with empty data for lot {lot_id}")
            return JsonResponse({'success': True, 'message': 'Preserved existing data'})

        # ENHANCED: Always save if JIG QR ID is provided
        if jig_qr_filled:
            print(f"💾 Force saving because JIG QR ID is filled: {data.get('jig_qr_id')}")
        ...
        print(f"{'✅ Created' if created else '🔄 Updated'} auto-save for lot {lot_id}")
...
# Check and clean expired auto-saves
        try:
            expired_saves = JigAutoSave.objects.filter(
                user=request.user,
                lot_id=lot_id
            ).filter(
                models.Q(updated_at__lt=timezone.now() - timedelta(hours=24))
            )
            if expired_saves.exists():
                expired_saves.delete()
                logger.info(f"Cleaned {expired_saves.count()} expired auto-saves for user {request.user} lot {lot_id}")
        except Exception as e:
            logger.warning(f"Error cleaning expired auto-saves: {e}")
