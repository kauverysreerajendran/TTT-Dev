...
class JigPickTableView(APIView):
    ...
    def get(self, request):
        ...
        # Get JigLoadingMaster data for all objects
        jig_loading_lookup = {}
        for jlm in JigLoadingMaster.objects.select_related('model_stock_no').all():
            if jlm.model_stock_no and jlm.model_stock_no.model_no:
                jig_loading_lookup[jlm.model_stock_no.model_no] = jlm
        ...
        for master_obj in page_obj.object_list:
            ...
            jlm = None
            if master_obj.model_stock_no and master_obj.model_stock_no.model_no:
                jlm = jig_loading_lookup.get(master_obj.model_stock_no.model_no)
...
def calculate_half_filled_trays_by_capacity(stock_lot_id, display_qty, jig_capacity, tray_capacity, force_original_qty=None):
    ...
    - Show remaining quantity that can fit in remaining jig capacity
    - Only show if it's less than a full tray (truly half-filled)
    """
    half_filled_trays = []
    try:
        print(f"HALF FILLED CALCULATION (FIXED FOR JIG CAPACITY):")
        ...
        # Calculate complete trays that will be used in delink
        ...
        # Calculate remaining jig capacity after delink
        remaining_jig_capacity = jig_capacity - actual_delink_qty
        print(f"  Remaining jig capacity: {remaining_jig_capacity}")

        if remaining_jig_capacity <= 0:
            print("  No remaining jig capacity, no half filled trays needed")
            return half_filled_trays

        # Calculate remaining quantity from original
        remaining_qty = original_qty - actual_delink_qty
        print(f"  Total remaining qty: {remaining_qty}")

        if remaining_qty <= 0:
            print("  No remaining quantity, no half filled trays needed")
            return half_filled_trays

        # Quantity that can be loaded in remaining jig capacity
        can_load = min(remaining_qty, remaining_jig_capacity)
        print(f"  Can load in remaining capacity: {can_load}")

        # Only show as half-filled if it's less than a full tray
        if can_load > 0 and can_load < tray_capacity:
            half_filled_trays.append({
                'tray_id': f"REMAINING-{can_load}",
                'tray_quantity': can_load,
                'original_tray_quantity': tray_capacity,
                'is_top_tray': True,
                'lot_id': stock_lot_id,
                'theoretical': True
            })
            print(f"  ✓ HALF-FILLED TRAY: {can_load} pieces (less than full tray)")
        else:
            print(f"  No half-filled tray needed (can_load={can_load}, tray_capacity={tray_capacity})")
...
def fetch_jig_related_data(request):
    ...
    try:
        ...
        jig_master = JigLoadingMaster.objects.filter(model_stock_no__model_no=jig_model_stock_no.model_no).first() if jig_model_stock_no and jig_model_stock_no.model_no else None
        ...
        print(f"🎯 FINAL API RESPONSE display_qty: {base_qty}")
    ...
    def post(self, request):
        ...
        try:
            ...
            if not is_draft:
                ...
                # Faulty slots validation (capacity-based) - Dynamic calculation
                # Allow 5% of jig capacity for faulty slots, with minimum 3 and maximum 12
                faulty_limit = max(3, min(12, int(jig_master.jig_capacity * 0.05)))
...
class JigDetailsValidateQRAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if exists:
                # Extract jig capacity from jig_qr_id (numeric part at the end)
                match = re.search(r'(\d+)$', jig_qr_id)
                jig_capacity = int(match.group(1)) if match else 0
...
class JIGTrayIdList_Complete_APIView(APIView):
    def post(self, request):
        ...
        loaded_qty = request.data.get('loaded_qty')
        
        # Get stock model
        stock_model, is_recovery, original_qty, tray_capacity, model_stock_no = get_stock_model_data(lot_id)
        
        # Use loaded_qty if provided, else original_qty
        required_qty = loaded_qty if loaded_qty else original_qty
        
        # Get jig loading master
        jig_loading_master = JigLoadingMaster.objects.filter(model_stock_no__model_no=model_stock_no).first()
        if not jig_loading_master:
            return Response({'error': 'Jig loading master not found'}, status=400)
        
        # Generate distribution
        delink_trays = generate_optimal_tray_distribution_by_capacity(required_qty, tray_capacity, lot_id)
        half_filled_trays = calculate_half_filled_trays_by_capacity(lot_id, required_qty, jig_loading_master.jig_capacity, tray_capacity)
