    ...
    def get(self, request):
        ...
        for data in master_data:   
            ...
            is_delink_only = (nq_physical_qty > 0 and 
                              brass_rejection_total_qty >= nq_physical_qty and 
...
class NQ_Zone_BatchRejectionAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print(f"✅ [NQ_Zone_BatchRejectionAPIView] Updated {updated_unload_trays_count} JigUnload_TrayId records")
            ...
            for tray_id in unique_tray_ids:
                ...
                if jig_tray_obj or tray_obj:
                    ...
                    if nickel_qc_tray_obj:
                        ...
                        # Create new NickelQcTrayId record
                        ...
                        print(f"➕ [NQ_Zone_TrayRejectionAPIView] Created new NickelQcTrayId for tray_id={tray_id}, tray_quantity={tray_total_qty}")
...
def nq_zone_get_delink_tray_data(request):
    ...
    try:
        ...
        rejections = Nickel_QC_Rejected_TrayScan.objects.filter(lot_id=unload_lot_id)
        ...
        for i, qty in enumerate(current_distribution):
            if qty == 0:
                ...
                if original_capacity > 0:
                    ...
                    print(f"✅ [nq_zone_get_delink_tray_data] Empty tray found: position {i+1}, original capacity: {original_capacity}")
        
        print(f"🔍 [nq_zone_get_delink_tray_data] Empty tray positions: {empty_tray_positions}")
        print(f"🔍 [nq_zone_get_delink_tray_data] Total empty trays needing delink: {len(delink_trays)}")
        
        if len(delink_trays) == 0:
            print("ℹ️ [nq_zone_get_delink_tray_data] No empty trays found - no delink needed")
            return Response({
                'success': True,
                'delink_trays': [],
...
class NQ_Zone_TrayRejectionDraftAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            print(f"🔍 [NQ_Zone_TrayRejectionDraftAPIView] Searching for JigUnloadAfterTable record...")
            ...
            JigUnloadAfterTable.objects.filter(lot_id=unload_lot_id).update(nq_draft=True)
