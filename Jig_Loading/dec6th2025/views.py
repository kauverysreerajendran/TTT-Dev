...
class JigAddModalDataView(TemplateView):
    ...
    def _prepare_modal_data(self, request, batch, model_master, stock, jig_qr_id, lot_id, broken_hooks=0):
        ...
        if broken_hooks > 0:
            ...
            # No broken hooks - need to consider jig capacity for new jigs
            # Get plating stock number and jig capacity first
            plating_stk_no = self._resolve_plating_stock_number(batch, model_master)
            if plating_stk_no:
                jig_master = JigLoadingMaster.objects.filter(model_stock_no__plating_stk_no=plating_stk_no).first()
                if jig_master:
            ...
            if jig_details:
                # Existing jig - use full lot quantity
                modal_data['effective_loaded_cases'] = original_lot_qty
                modal_data['remaining_cases'] = 0
            else:
                # New jig - cap at jig capacity
                modal_data['effective_loaded_cases'] = min(original_lot_qty, jig_capacity)
                modal_data['remaining_cases'] = max(0, original_lot_qty - jig_capacity)
        
        # Handle broken hooks case by updating jig capacity if not already set
        if broken_hooks > 0 and modal_data.get('jig_capacity', 0) == 0:
            plating_stk_no = self._resolve_plating_stock_number(batch, model_master)
            if plating_stk_no:
                jig_master = JigLoadingMaster.objects.filter(model_stock_no__plating_stk_no=plating_stk_no).first()
                if jig_master:
        ...
        tray_capacity = batch.tray_capacity if batch and batch.tray_capacity else None
        if not tray_capacity:
            raise ValueError(f"Tray capacity not configured for batch {batch.batch_id if batch else 'None'}")
        ...
        if jig_details:
            ...
            modal_data['loaded_cases_qty'] = 0  # Start with 0, will be updated as trays are scanned
            modal_data['loaded_hooks'] = 0  # Start with 0, will be updated as trays are scanned
            # --- FIX: Only allow empty_hooks > 0 if lot qty < jig capacity, else always 0 ---
            if modal_data['effective_loaded_cases'] < modal_data['jig_capacity']:
                modal_data['empty_hooks'] = modal_data['jig_capacity'] - modal_data['effective_loaded_cases']
            else:
                modal_data['empty_hooks'] = 0
            # --- END FIX ---
        ...
        if broken_hooks > 0:
            ...
            # No broken hooks - check if lot qty > jig capacity for remaining cases
            original_lot_qty = stock.total_stock or 0
            if original_lot_qty > modal_data['jig_capacity']:
                leftover_cases = original_lot_qty - modal_data['jig_capacity']
        ...
        modal_data['tray_distribution'] = self._calculate_tray_distribution(
            ...
            batch,
            original_lot_qty  # Pass original lot quantity
        )
        ...
        # Auto-save to draft table if there are remaining cases (half-filled tray scenario)
        if request.user.is_authenticated and modal_data['remaining_cases'] > 0:
            batch_id_str = batch.batch_id if batch else ''
            if batch_id_str and lot_id:
                try:
                    draft, created = JigLoadingManualDraft.objects.get_or_create(
                        batch_id=batch_id_str,
                        lot_id=lot_id,
                        user=request.user,
                        defaults={
                            'draft_data': ui_config,
                            'jig_cases_remaining_count': modal_data['remaining_cases'],
                            'updated_lot_qty': modal_data['jig_capacity']
                        }
                    )
                    if not created:
                        # Update existing draft
                        draft.jig_cases_remaining_count = modal_data['remaining_cases']
                        draft.updated_lot_qty = modal_data['jig_capacity']
                        draft.draft_data = ui_config
                        draft.save()
                    logger.info(f"📝 Auto-saved draft for remaining cases: {modal_data['remaining_cases']}")
    ...
    def _calculate_tray_distribution(self, loaded_cases_qty, jig_capacity, broken_hooks, batch, original_lot_qty=None):
        """
        ...
        if broken_hooks > 0:
            ...
            # Half-filled tray scenario: lot qty > jig capacity
            tray_distribution_cases = min(loaded_cases_qty, jig_capacity)
            # Calculate remaining cases that exceed jig capacity
            if original_lot_qty is None:
                original_lot_qty = loaded_cases_qty
            half_filled_cases = max(0, original_lot_qty - jig_capacity)
        ...
        if half_filled_cases > 0:
            half_filled_distribution = self._distribute_cases_to_trays(half_filled_cases, tray_capacity)
    ...
    def _prepare_existing_delink_table(self, lot_id, batch, effective_loaded_cases, tray_capacity, broken_hooks):
        ...
        # Get existing trays for this lot AND batch (NOT filtering by delink_tray=True)
        ...
        if existing_trays.exists():
            ...
            # When broken hooks > 0, calculate effective tray distribution
            if broken_hooks > 0:
                logger.info(f"🔧 Processing broken hooks scenario: broken_hooks={broken_hooks}, effective_cases={effective_loaded_cases}")
                
                # Calculate broken hooks tray distribution and update DB
                effective_tray_data = self._calculate_broken_hooks_tray_distribution(lot_id, effective_loaded_cases, broken_hooks, batch)
                
                # Only show effective trays in delink table
                for tray_data in effective_tray_data:
                    delink_table.append({
                        ...
                        'original_quantity': tray_data['original_qty'],  # For reference
                        'excluded_quantity': tray_data['excluded_qty']   # For half-filled tracking
                    })
                    
            else:
                # No broken hooks - calculate distribution based on effective_loaded_cases
                ...
                num_full = effective_loaded_cases // tray_capacity
                remainder = effective_loaded_cases % tray_capacity
                
                if remainder > 0:
                    calculated_qtys = [remainder] + [tray_capacity] * num_full
                else:
                    calculated_qtys = [tray_capacity] * num_full
                
                logger.info(f"📊 Calculated qtys: {calculated_qtys}")
                
                # Assign to existing trays
                for idx, tray in enumerate(existing_trays):
                    if idx < len(calculated_qtys):
                        qty = calculated_qtys[idx]
                        # Update effective qty
                        tray.broken_hooks_effective_tray = True
                        tray.broken_hooks_excluded_qty = 0
...
class DelinkTableAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Get jig capacity from JigLoadingMaster using the same lookup method as modal API
        jig_capacity = 0
        if model_master:
            # Get plating stock number first
            plating_stk_no = ''
            if batch and batch.plating_stk_no:
                plating_stk_no = batch.plating_stk_no
            elif batch and batch.model_stock_no and batch.model_stock_no.plating_stk_no:
            ...
            # Use same lookup method as modal API
            if plating_stk_no:
                jig_master = JigLoadingMaster.objects.filter(model_stock_no__plating_stk_no=plating_stk_no).first()
                if jig_master:
                    jig_capacity = jig_master.jig_capacity
                    logger.info(f"🔧 Jig capacity from JigLoadingMaster (by plating_stk_no): {jig_capacity}")
                else:
                    logger.warning(f"⚠️ JigLoadingMaster not found for plating_stk_no: {plating_stk_no}")
                    jig_capacity = loaded_cases_qty  # Use loaded cases qty as fallback
            else:
                # Fallback to direct model lookup
                try:
                    jig_master = JigLoadingMaster.objects.get(model_stock_no=model_master)
                    jig_capacity = jig_master.jig_capacity
                    logger.info(f"🔧 Jig capacity from JigLoadingMaster (direct lookup): {jig_capacity}")
        ...
        if tray_capacity > 0 and actual_qty > 0:
            if broken_hooks == 0:
                ...
                if remainder_qty > 0:
                    qtys = [remainder_qty] + [tray_capacity] * num_full_trays
                else:
                    qtys = [tray_capacity] * num_full_trays
                
                total_trays = len(qtys)
                ...
                for i in range(total_trays):
                    ...
                    if existing_tray:
                        ...
                        tray_quantity = tray_qty  # Use calculated qty
                        placeholder = f"Scan {tray_id}"
                        readonly = True
                    else:
                        # SKIP - don't create fake tray entries, only show actual scanned trays
                        logger.warning(f"⚠️ No existing tray found at index {i}, skipping fake creation")
                        continue
...
class JigLoadingManualDraftAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        # Get jig_capacity similar to modal API
        jig_capacity = 0
        try:
            from modelmasterapp.models import ModelMasterCreation
            batch = ModelMasterCreation.objects.get(batch_id=batch_id)
            model_master = batch.model_stock_no
            plating_stk_no = batch.plating_stk_no or (model_master.plating_stk_no if model_master else '')
            if plating_stk_no:
                jig_master = JigLoadingMaster.objects.filter(model_stock_no__plating_stk_no=plating_stk_no).first()
                if jig_master:
                    jig_capacity = jig_master.jig_capacity
            ...
            logger.error(f"❌ Error getting jig_capacity: {e}")

        # Get original lot qty
        original_lot_qty = 0
        try:
            ...
            logger.error(f"❌ Error getting original lot qty: {e}")

        # Calculate loaded_cases and half_filled_cases from draft_data
        trays = draft_data.get('trays', [])
        loaded_cases = sum(int(t.get('tray_qty', 0)) for t in trays if not str(t.get('row_index', '')).startswith('half_'))
        half_filled_cases = max(0, original_lot_qty - jig_capacity)  # Total half-filled cases

        logger.info(f"📊 Calculated: loaded_cases={loaded_cases}, half_filled_cases={half_filled_cases}, jig_capacity={jig_capacity}")

        # Filter only scanned trays (those with tray_id)
        scanned_trays = [t for t in trays if t.get('tray_id', '').strip()]
        ...
        obj, created = JigLoadingManualDraft.objects.update_or_create(
            ...
            defaults={
                ...
                'jig_cases_remaining_count': half_filled_cases,
                'updated_lot_qty': jig_capacity,  # Use jig_capacity instead of loaded_cases sum
                'original_lot_qty': original_lot_qty,
                'tray_info': scanned_trays,
            }
        )
...
class JigSaveAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Main lot gets the updated lot qty from draft (jig capacity) or calculate based on jig capacity
            if draft and draft.updated_lot_qty:
                total_cases_loaded = draft.updated_lot_qty
            else:
                # When no draft, ensure main lot doesn't exceed jig capacity
                total_cases_loaded = min(effective_total_cases, jig_capacity) if jig_capacity else effective_total_cases
            ...
            # --- CALCULATE REMAINING CASES FIRST (before using in jig_detail_data) ---
            remaining_cases = 0
            if draft and draft.jig_cases_remaining_count:
                remaining_cases = draft.jig_cases_remaining_count
            elif effective_total_cases > jig_capacity:
                remaining_cases = effective_total_cases - jig_capacity
            ...
            jig_detail_data = {
                ...
                'jig_type': jig_type,
                'jig_capacity': jig_capacity,
                'total_cases_loaded': total_cases_loaded,  # Use the calculated total_cases_loaded (98)
                'empty_slots': 0,
                'faulty_slots': 0,
                'broken_hooks': 0,  # Set to 0 since broken are in separate lot
                'no_of_model_cases': [draft_data.get('plating_stk_no', '')],
                'new_lot_ids': [lot_id],
                'lot_id_quantities': {lot_id: total_cases_loaded},
                'lot_id': lot_id,
                'draft_save': False,
                ...
                'jig_cases_remaining_count': remaining_cases,
                'updated_lot_qty': total_cases_loaded,
                'original_lot_qty': original_stock.total_stock,
                'tray_info': draft.tray_info if draft else valid_trays,
            }
            ...
            num_full_trays = total_cases_loaded // tray_capacity
            remainder_qty = total_cases_loaded % tray_capacity
            ...
            # --- CREATE REMAINING LOT (for cases exceeding jig capacity) ---
            if not data.get('draft', False) and remaining_cases > 0:
                new_lot_id = f"LID{timezone.now().strftime('%d%m%Y%H%M%S')}{remaining_cases:04d}"

                # Create TotalStockModel for remaining lot
                remaining_stock = TotalStockModel.objects.create(
                    batch_id=batch_obj,
                ...
                # Create tray for remaining cases
                if remaining_cases > 0:
                    # Generate unique tray ID for remaining cases
                    remaining_tray_id = f"JB-R{timezone.now().strftime('%Y%m%d%H%M%S')}" 
                    
                    JigLoadTrayId.objects.create(
                        lot_id=new_lot_id,
                        tray_id=remaining_tray_id,
                ...
                # Create JigDetails for remaining lot
                remaining_jig_detail_data = {
                    'jig_qr_id': f"{jig_qr_id}-REMAIN",
                    'jig_type': jig_type,
                    'jig_capacity': jig_capacity,
                    ...
                    'no_of_cycle': int(draft_data.get('no_of_cycle', 1)),
                    'jig_cases_remaining_count': remaining_cases,
                    'updated_lot_qty': remaining_cases,
                    'original_lot_qty': original_stock.total_stock,
                    'tray_info': [{'tray_id': f"JB-R{timezone.now().strftime('%Y%m%d%H%M%S')}", 'tray_qty': remaining_cases}],
                }
                JigDetails.objects.create(**remaining_jig_detail_data)

            # --- CREATE BROKEN LOT (35 cases) ---
            broken_cases = broken_hooks_count
            
            if broken_cases > 0:
                try:
                    new_lot_id = f"LID{timezone.now().strftime('%d%m%Y%H%M%S')}{broken_cases:04d}"
                    ...
                    broken_jig_detail_data = {
                        ...
                        'jig_type': jig_type,
                        'jig_capacity': jig_capacity,
                        'total_cases_loaded': broken_cases,
                        'empty_slots': 0,
                        'faulty_slots': 0,
                        'broken_hooks': broken_hooks_count,
                        'no_of_model_cases': [draft_data.get('plating_stk_no', '')],
                        'new_lot_ids': [new_lot_id],
                        'lot_id_quantities': {new_lot_id: broken_cases},
                        'lot_id': new_lot_id,
                        'draft_save': False,
                        'created_by': request.user,
                        'ep_bath_type': data.get('ep_bath_type', 'Bright'),
                    ...
                    broken_cases = 0  # Reset to skip further processing
...
class JigCompletedTable(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for stock in completed_qs:
            ...
            # --- FIX START: Use updated_lot_qty from JigLoadingManualDraft if lot qty > jig capacity ---
            updated_lot_qty = None
            try:
                draft = JigLoadingManualDraft.objects.filter(lot_id=stock.lot_id).order_by('-updated_at').first()
                if draft and draft.updated_lot_qty:
                    updated_lot_qty = draft.updated_lot_qty
            except Exception:
                updated_lot_qty = None

            # Use updated_lot_qty if it exists and is less than original lot qty
            if updated_lot_qty and jig_capacity and stock.total_stock > jig_capacity:
                lot_qty = updated_lot_qty
            else:
