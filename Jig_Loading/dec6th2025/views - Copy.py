...
class JigAddModalDataView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return JsonResponse({
            ...
            'half_filled_enabled': modal_data.get('half_filled_enabled'),
            'half_filled_qty': modal_data.get('half_filled_qty')
        })
    ...
    def _prepare_modal_data(self, request, batch, model_master, stock, jig_qr_id, lot_id, broken_hooks=0):
        ...
        # Set initial loaded_cases_qty from stock minus broken hooks
        modal_data['loaded_cases_qty'] = (stock.total_stock or 0) - broken_hooks
        ...
        if plating_stk_no:
            ...
            if jig_master:
                ...
                modal_data['jig_capacity'] = modal_data['loaded_cases_qty']
        else:
            modal_data['jig_capacity'] = modal_data['loaded_cases_qty']

        # Get tray capacity from TrayType
        try:
            tray_type_obj = TrayType.objects.get(tray_type=modal_data['tray_type'])
            tray_capacity = tray_type_obj.tray_capacity
        except TrayType.DoesNotExist:
            tray_capacity = 0  # No hardcode, use 0 if not found
        ...
        if jig_details:
            ...
            modal_data['broken_buildup_hooks'] = broken_hooks or jig_details.broken_hooks
            modal_data['loaded_cases_qty'] = (jig_details.total_cases_loaded or 0) - broken_hooks
            modal_data['loaded_hooks'] = jig_details.total_cases_loaded or 0
            ...
            modal_data['loaded_cases_qty'] = (stock.total_stock or 0) - broken_hooks
            modal_data['loaded_hooks'] = stock.total_stock or 0
            # --- FIX: Only allow empty_hooks > 0 if lot qty < jig capacity, else always 0 ---
            if modal_data['loaded_cases_qty'] < modal_data['jig_capacity']:
                modal_data['empty_hooks'] = modal_data['jig_capacity'] - modal_data['loaded_cases_qty']
            else:
        ...
        # Half filled tray logic
        if broken_hooks > 0:
            modal_data['half_filled_enabled'] = True
            modal_data['half_filled_qty'] = modal_data['loaded_cases_qty'] % tray_capacity if tray_capacity else 0
        else:
            modal_data['half_filled_enabled'] = False
            modal_data['half_filled_qty'] = 0
        # Force empty_hooks to 0 when broken_hooks > 0
        if broken_hooks > 0:
            modal_data['empty_hooks'] = 0
        ...
        is_draft = jig_details and jig_details.draft_save
        modal_data['delink_table'] = self._prepare_existing_delink_table(lot_id, batch, modal_data['loaded_cases_qty'], tray_capacity, is_draft)
        ...
        modal_data['logs'] = {
            ...
            'half_filled_enabled': modal_data['half_filled_enabled'],
            'half_filled_qty': modal_data['half_filled_qty']
        }
    ...
    def _prepare_existing_delink_table(self, lot_id, batch, loaded_cases_qty, tray_capacity, is_draft=False):
        """
        Prepare existing delink table data (for display only, not calculation).
        For new jigs, show empty placeholders for manual scanning.
        For drafts, return empty list as trays are restored from draft data.
        """
        if is_draft:
            return []
        num_full_trays = loaded_cases_qty // tray_capacity
        delink_table = []
        for idx in range(num_full_trays):
            delink_table.append({
                's_no': idx + 1,
                'tray_id': '',  # Empty for manual scanning
    ...
    try:
        ...
        if drafted_jig_current_batch:
            ...
            if (
                ...
                getattr(drafted_jig_current_batch, 'lot_id', None) == request.data.get('lot_id', '').strip()
            ):
...
class JigSaveAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # --- FIX: Always store delink tray data from request if present ---
            delink_tray_data = []
            # 1. If present in request, use it (frontend should send this on Save)
            if 'delink_tray_data' in data and isinstance(data['delink_tray_data'], list) and data['delink_tray_data']:
                delink_tray_data = data['delink_tray_data']
            else:
                # 2. Else, fallback to latest draft (legacy/compatibility)
                try:
                    draft = JigLoadingManualDraft.objects.filter(lot_id=lot_id).order_by('-updated_at').first()
                    if draft and draft.draft_data and 'trays' in draft.draft_data:
                        delink_tray_data = draft.draft_data['trays']
                except Exception:
                    pass
            ...
            jig_detail_data = {
                ...
                'broken_hooks': data.get('broken_hooks', 0),
                ...
                'ep_bath_type': 'Bright',
                'plating_color': 'Default',
                'forging': forging_info,
                'delink_tray_data': delink_tray_data,  # <-- always store here
            }
...
# Complete Table - Fetch Tray IDs for a Lot (View Icon)   
@require_GET
def jig_tray_id_list(request):
    """
    API endpoint to return all delinked tray IDs for a given lot (for completed table view icon).
    If no delinked trays are found in JigLoadTrayId, fallback to the latest JigDetails.delink_tray_data.
    """
    lot_id = request.GET.get('stock_lot_id')
    if not lot_id:
        return JsonResponse({'success': False, 'error': 'Missing lot_id'}, status=400)

    # 1. Try to fetch trays that are delinked for this lot (normal case)
    trays = JigLoadTrayId.objects.filter(lot_id=lot_id, delink_tray=True)
    tray_list = [
        {
            'tray_id': t.tray_id,
    ...
    # 2. If not found, fallback to latest JigDetails.delink_tray_data (from manual draft/final save)
    if not tray_list:
        jig_detail = JigDetails.objects.filter(lot_id=lot_id).order_by('-date_time').first()
        if jig_detail and jig_detail.delink_tray_data:
            # delink_tray_data is expected to be a list of dicts with tray_id and tray_qty
            tray_list = [
                {
    ...
    return JsonResponse({'success': True, 'trays': tray_list, 'count': len(tray_list)})
