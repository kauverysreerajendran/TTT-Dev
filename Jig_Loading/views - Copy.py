...
class JigAddModalDataView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if excess > 0:
            excess_message = f"{excess} cases are in excess"
        else:
            excess_message = ""
        ...
        logger.info(f"📊 Lot details: lot_id={lot_id}, total_stock={stock.total_stock}, excess={excess}, excess_message='{excess_message}'")
    ...
    def _calculate_tray_distribution(self, loaded_cases_qty, jig_capacity, broken_hooks, batch, excess=0):
        """
