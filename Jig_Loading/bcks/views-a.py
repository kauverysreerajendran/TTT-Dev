...
class JigDetailsSaveAPIView(APIView):
    ...
    def process_half_filled_tray_data(self, half_filled_tray_data):
        """
        Process half-filled tray data by matching user-scanned tray IDs to lot IDs
        """
        if not half_filled_tray_data:
            return []
            
        processed_data = []
        for entry in half_filled_tray_data:
            tray_id = entry.get('tray_id', '').strip()
            lot_id = entry.get('lot_id', '').strip()
            
            # If lot_id is empty, try to find it from database
            if tray_id and not lot_id:
                try:
                    # Look for existing tray records to find the lot_id
                    tray_record = JigLoadTrayId.objects.filter(tray_id=tray_id, tray_quantity__gt=0).first()
                    if not tray_record:
                        # Fallback to master TrayId (may be present without a JigLoadTrayId entry)
                        from modelmasterapp.models import TrayId
                        tray_record = TrayId.objects.filter(tray_id=tray_id).first()
                    
                    if tray_record and getattr(tray_record, 'lot_id', None):
                        lot_id = tray_record.lot_id
                        print(f"🔗 Matched user tray {tray_id} ({entry.get('tray_quantity')} pieces) to lot {lot_id}")
                except Exception as e:
                    print(f"⚠️ Error matching tray {tray_id}: {e}")
            
            # Add to processed data with lot_id filled
            processed_entry = entry.copy()
            processed_entry['lot_id'] = lot_id
    ...
    def calculate_lot_quantities_and_filter_lots(self, lot_ids, lot_id_quantities, delink_tray_data, half_filled_tray_data, is_draft, primary_lot_id):
        ...
        # Process half_filled_tray_data to fill empty lot_ids
        processed_half_filled_tray_data = self.process_half_filled_tray_data(half_filled_tray_data)
        ...
        if delink_tray_data:
            ...
            # PATCH: Also include lots from PROCESSED half_filled_tray_data
            half_filled_lot_ids = set(entry.get('lot_id', '').strip() for entry in processed_half_filled_tray_data if entry.get('lot_id', '').strip())
            for lot_id in half_filled_lot_ids:
                if lot_id and lot_id not in lot_ids:
                    ...
                    recalculated_lot_id_quantities[lot_id] = sum(int(entry.get('tray_quantity', 0)) for entry in processed_half_filled_tray_data if entry.get('lot_id', '').strip() == lot_id)
            # --- PATCH END ---
        else:
    ...
    def post(self, request):
        ...
        try:
            ...
            # Process half_filled_tray_data to assign lot_ids
            processed_half_filled_tray_data = self.process_half_filled_tray_data(half_filled_tray_data)
            ...
            try:
                ...
                if not jig_master:
                    return JsonResponse({
                        'success': False, 
                        'error': f'Jig loading master data not found for model: {model_stock_no.model_no}'
                    }, status=404)
            except Exception as e:
                return JsonResponse({
                    'success': False, 
            ...
            jig_details_data = {
                ...
                'broken_hooks': data.get('broken_hooks', 0),  # Add broken_hooks with default 0
                ...
                'forging': getattr(jig_master, 'forging_info', '') or '',  # Add forging field
                ...
                'half_filled_tray_data': processed_half_filled_tray_data,  # Use processed data
            }
            ...
            if not is_draft:
                ...
                if processed_half_filled_tray_data:
                    print(f"🔧 Processing {len(processed_half_filled_tray_data)} half filled tray entries")
                    
                    for half_entry in processed_half_filled_tray_data:
                ...
                half_filled_tray_ids = [entry.get('tray_id', '').strip() for entry in processed_half_filled_tray_data if entry.get('tray_id', '').strip()]
...
def validate_tray_id(request):
    ...
    # HALF-FILLED CASE: Only allow tray IDs that exist in TrayId master (must be real tray)
    if is_half_filled:
        # Import TrayId from modelmasterapp
        from modelmasterapp.models import TrayId
        tray_any = TrayId.objects.filter(tray_id=tray_id).first()
        if not tray_any:
            # Tray does not exist in master, do not allow
            print(f"[validate_tray_id] half-filled tray not found in TrayId master: {tray_id}")
            return JsonResponse({'exists': False, 'lot_match': False, 'error': 'Tray ID not found in master. Only real trays are allowed.'})
        ...
        if not lot_id:
            ...
            # Allow if tray has no assigned lot (unassigned) or matches requested lot_id
            lot_match = (getattr(tray_any, 'lot_id', None) in [None, '']) or (str(getattr(tray_any, 'lot_id', '')) == str(lot_id))
...
class JIGTrayIdList_Complete_APIView(APIView):
    def get(self, request):
        ...
        for tray in other_trays:
            # Check if this is a delinked tray with partial usage
            if getattr(tray, 'delink_tray', False):
                # Find the JigDetails with delink_tray_data for this tray
                jig_detail = JigDetails.objects.filter(
                    Q(lot_id=lot_id) | Q(new_lot_ids__contains=[lot_id]),
                    delink_tray_data__isnull=False
                ).first()
                
                if jig_detail and jig_detail.delink_tray_data:
                    # Find the specific tray entry in delink_tray_data
                    for delink_entry in jig_detail.delink_tray_data:
                        if (delink_entry.get('tray_id') == tray.tray_id and 
                            delink_entry.get('lot_id') == lot_id):
                            expected_usage = delink_entry.get('expected_usage', 12)
                            standard_capacity = 12  # Standard tray capacity
                            
                            # If partial usage, add leftover as new tray first
                            if expected_usage < standard_capacity:
                                leftover_qty = standard_capacity - expected_usage
                                data.append({
                                    's_no': row_counter,
                                    'tray_id': f"{tray.tray_id}-NEW",
                                    'tray_quantity': leftover_qty,
                                    'position': row_counter - 1,
                                    'is_top_tray': False,
                                    'rejected_tray': False,
                                    'delink_tray': False,
                            ...
                            # Add delinked tray with actual used quantity
                            tray_data = create_tray_data(tray, is_top=False)
                            tray_data['tray_quantity'] = expected_usage  # Use actual used quantity
                            tray_data['status'] = 'DELINKED'
                            data.append(tray_data)
                            row_counter += 1
                            break
                    else:
                        # No specific entry found, use default
                        data.append(create_tray_data(tray, is_top=False))
                        row_counter += 1
                else:
                    # No delink data found, use default
                    data.append(create_tray_data(tray, is_top=False))
                    row_counter += 1
            else:
                # Regular tray (not delinked)
                data.append(create_tray_data(tray, is_top=False))
                row_counter += 1
