...
def generate_optimal_tray_distribution_by_capacity(required_qty, tray_capacity, lot_id, available_tray_ids=None, force_original_qty=None):
    ...
    try:
        ...
        # Only include trays that are fully filled (actual_qty == tray_capacity)
        actual_tray_data = [(tray.tray_id, tray.tray_quantity) for tray in actual_trays if tray.tray_quantity == tray_capacity]
        print(f"Actual full tray data: {actual_tray_data}")
    ...
    # Use actual tray quantities for distribution - only full trays for delink
    total_used = 0
    trays_selected = 0
    for tray_id, actual_qty in actual_tray_data:
        if total_used + tray_capacity < required_qty:
            trays_needed.append({
                'tray_id': tray_id,
                'tray_quantity': tray_capacity,
                'used_quantity': tray_capacity,
            ...
            total_used += tray_capacity
            trays_selected += 1
            print(f"✓ Delink tray: {tray_id} using {tray_capacity}/{tray_capacity} pieces (complete)")
        else:
            remainder = required_qty - total_used
            if remainder > 0:
                trays_needed.append({
                    'tray_id': tray_id,
                    'tray_quantity': tray_capacity,
                ...
                total_used += remainder
                trays_selected += 1
                print(f"✓ Delink tray: {tray_id} using {remainder}/{tray_capacity} pieces (partial)")
            break  # <-- CRITICAL: Stop after required_qty is reached

    total_distributed = sum(tray['used_quantity'] for tray in trays_needed)
    print(f"Final delink distribution: {len(trays_needed)} trays totaling {total_distributed} pieces (selected {trays_selected} trays)")
...
def generate_multi_model_optimal_distribution(lot_quantities_dict, tray_capacity_dict, jig_capacity=0, broken_hooks=0):
    ...
    try:
        ...
        for lot_id, required_qty in lot_quantities_dict.items():
            ...
            try:
                ...
                for tray in actual_trays:
                    # Only include full trays (tray_quantity == tray_capacity)
                    if tray.tray_quantity == tray_capacity:
                        usable_qty = tray_capacity
                        all_trays.append({
        ...
        # Sort all trays by lot order (all are full trays now)
        all_trays.sort(key=lambda x: list(lot_quantities_dict.keys()).index(x['lot_id']))
        ...
        for lot_id in lot_order:
            ...
            for tray in lot_trays:
                ...
                if usable == tray['usable_qty']:
                    # Full tray allocation - add to delink
                    tray['allocated_qty'] = usable
                    selected_trays.append(tray)
                    status = "complete"
                else:
                    # Partial tray allocation - add to half-filled instead of delink
                    plating_stk_no = all_lot_data[lot_id]['plating_stk_no']
                    half_filled_tray = {
                        'tray_id': tray['tray_id'],
                        'tray_quantity': usable,
                        'original_tray_quantity': tray['usable_qty'],
                        'is_top_tray': True,
                        'lot_id': tray['lot_id'],
                        'plating_stk_no': plating_stk_no,
                        'theoretical': True,
                        ...
                        'allocated_qty': usable
                    }
                    result['half_filled_trays'].append(half_filled_tray)
                    status = "partial (half-filled)"
                
                allocated += usable
                remaining_jig -= usable
                total_allocated += usable
                
                print(f"TRAY ALLOCATION: {tray['tray_id']} ({tray['lot_id']}) - Used: {usable}/{tray['usable_qty']} pieces ({status})")
...
class JigDetailsSaveAPIView(APIView):
    ...
    def post(self, request):
            ...
            try:
                ...
                # Minimal fix: recalculate lot_id_quantities from tray data
                corrected_lot_id_quantities = {lot_id: 0 for lot_id in lot_ids}
                for tray in delink_tray_data:
                    lot_id = tray.get('lot_id')
                    qty = int(tray.get('expected_usage', 0) or 0)
                    if lot_id in corrected_lot_id_quantities:
                        corrected_lot_id_quantities[lot_id] += qty
                for tray in half_filled_tray_data:
                    lot_id = tray.get('lot_id')
                    qty = int(tray.get('tray_quantity', 0) or 0)
                    if lot_id in corrected_lot_id_quantities:
                        corrected_lot_id_quantities[lot_id] += qty
                lot_id_quantities = corrected_lot_id_quantities
                total_cases_loaded = recalculated_total_cases_loaded
                ...
                try:
                    ...
                    # === MINIMAL FIX: Correct total_cases_loaded calculation ===
                    # Calculate total_cases_loaded as sum of all tray quantities (including half-filled trays)
                    corrected_total_cases_loaded = 0
                    for tray in delink_tray_data:
                        qty = int(tray.get('expected_usage', 0) or 0)
                        corrected_total_cases_loaded += qty
                    for tray in half_filled_tray_data:
                        qty = int(tray.get('tray_quantity', 0) or 0)
                        corrected_total_cases_loaded += qty
                    # Cap at jig capacity
                    if corrected_total_cases_loaded > jig_master.jig_capacity:
                        corrected_total_cases_loaded = jig_master.jig_capacity
                    # Override total_cases_loaded
                    total_cases_loaded = corrected_total_cases_loaded
