...
class RecoveryBrassTrayId(models.Model):
    """
    RecoveryBrassTrayId Model
...
class RecoveryBrass_QC_Rejection_Table(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.rejection_reason_id:
            last = RecoveryBrass_QC_Rejection_Table.objects.order_by('-rejection_reason_id').first()
...
class RecoveryBrass_QC_Rejection_ReasonStore(models.Model):
    rejection_reason = models.ManyToManyField(RecoveryBrass_QC_Rejection_Table, blank=True)
    lot_id = models.CharField(max_length=50, null=True, blank=True, help_text="Lot ID")
...
class RecoveryBrass_QC_Draft_Store(models.Model):
...
class RecoveryBrass_TopTray_Draft_Store(models.Model):
    lot_id = models.CharField(max_length=255)
    batch_id = models.CharField(max_length=255)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
...
class RecoveryBrass_QC_Rejected_TrayScan(models.Model):
    lot_id = models.CharField(max_length=50, null=True, blank=True, help_text="Lot ID")
    ...
    rejection_reason = models.ForeignKey(RecoveryBrass_QC_Rejection_Table, on_delete=models.CASCADE)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    
    def __str__(self):
        return f"{self.rejection_reason} - {self.rejected_tray_quantity} - {self.lot_id}"

class RecoveryBrass_Qc_Accepted_TrayScan(models.Model):
    lot_id = models.CharField(max_length=50, null=True, blank=True, help_text="Lot ID")
...
class RecoveryBrass_Qc_Accepted_TrayID_Store(models.Model):
    lot_id = models.CharField(max_length=50, null=True, blank=True, help_text="Lot ID")
